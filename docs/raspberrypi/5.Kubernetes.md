...
spec:
  ...
  ports:
    - name: http
      port: 8080
  type: LoadBalancer # klipper-lb会创建svclb为前缀的DaemonSet，符合条件的所有节点都会被创建DaemonSet名为前缀的Pod来代理请求
```
...
# https://doc.traefik.io/traefik/v1.7/configuration/api/
    [api]
      dashboard = true
      entrypoint = "http" # 默认定义了http和https
...
# 访问http://{ip}:80/dashboard/，注意路径是“/dashboard/”。输入认证用户名test，密码test，即可进入traefik的dashboard
```
