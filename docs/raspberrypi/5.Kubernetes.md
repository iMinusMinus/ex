...
```sh
kubectl apply -f https://github.com/kubernetes/dashboard/blob/v2.1.0/aio/deploy/recommended.yaml
# 某些版本的部署脚本使用了被墙的域名k8s.gcr.io，可以将部署配置下载到本地后编辑替换成registry.aliyuncs.com/google_containers
# 查看运行状态，正确结果是namespace为kubernetes-dashboard下有状态为running的kubernetes-dashboard应用
